...
export default function Editor() {
  const { navTo, saveSession, sessions, editingSessionId, apiKeys, activeProvider, userProfile, backendUrl } = useTrance();
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      // If we have a Key, we can try direct (if supported), OR we default to backend if Key is missing
      const shouldUseBackend = !key || backendUrl !== "";

      if (shouldUseBackend) {
        // Use configured URL or fallback to the Vite proxy route '/generate'
        const endpoint = backendUrl ? `${backendUrl}/generate` : '/generate';
        
        console.log(`Attempting backend generation via: ${endpoint}`);
        ...
        const res = await fetch(endpoint, {
          ...
          body: JSON.stringify({ 
            ...
            api_key: key || undefined, // Backend will use env var if this is undefined
        ...
        // Direct Client Call (Fallback if key exists and user prefers explicit client-side)
        if (activeProvider === 'openai') {
          ...
          generatedText = data.choices[0].message.content;
            ...
            // Using CORS Proxy for DeepSeek if client-side
            const res = await fetch("https://corsproxy.io/?" + encodeURIComponent("https://api.deepseek.com/chat/completions"), {
            method: "POST",
            headers: { 
              "Content-Type": "application/json",
              "Authorization": `Bearer ${key}`
      ...
      // If backend failed and we have no key, prompt user
      if (!key) {
          navTo('PROFILE');
          showToast('Please check Backend connection or add API Key', 'info');
      }
    } finally {
