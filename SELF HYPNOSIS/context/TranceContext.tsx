...
// Safely access environment variables to prevent runtime crashes in some sandboxes
const getBackendUrl = () => {
  try {
    // @ts-ignore
...
export const TranceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [apiKeys, setApiKeys] = useState<Record<AIProvider, string>>(DEFAULT_API_KEYS);
  ...
  useEffect(() => {
    try {
      ...
      const storedKeys = localStorage.getItem('trance_api_keys');
      ...
      const storedBackend = localStorage.getItem('trance_backend_url');
      ...
      if (storedKeys) setApiKeys(prev => ({ ...prev, ...JSON.parse(storedKeys) }));
  ...
  useEffect(() => {
    ...
    localStorage.setItem('trance_api_keys', JSON.stringify(apiKeys));
    ...
    localStorage.setItem('trance_backend_url', backendUrl);
    localStorage.setItem('trance_settings', JSON.stringify(settings)); 
  }, [sessions, communitySessions, favorites, apiKeys, activeProvider, userProfile, backendUrl, settings]);
  ...
  const saveApiKey = (provider: AIProvider, key: string) => setApiKeys(prev => ({ ...prev, [provider]: key }));
  ...
  return (
    <TranceContext.Provider value={{
      ...
      apiKeys, activeProvider, userProfile, backendUrl,
      navTo, playSession, editSession, updateSettings, 
      saveApiKey, setActiveProvider, updateUserProfile, updateUserAnchors, setBackendUrl,
